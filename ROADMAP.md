...
- [ ] `drift-check` reporta divergencias, nunca corrige solo — **diseñado e implementado 2026-09-17, pendiente de deploy y de la primera corrida real contra la cuenta**. Corre en GitHub Actions (no pg_cron: Playwright no vive en Deno), cron semanal + botón "Chequear divergencias ahora". Compara TODA la cuenta de Zoom (no solo lo que la automatización creó) contra `meeting_occurrences` y guarda el resultado en `drift_check_runs`; si encuentra algo, además manda un email por Resend (reusa la cuenta de Galpón Digital/`ferreterias`). Selectores de lectura nuevos (`listMeetingIds`, `readMeetingSummary` en `zoom-browser.ts`) sin verificar contra el DOM real todavía — ver `ZOOM_AUTOMATION.md` y `PROGRESS.md` 2026-09-17 para el detalle y los pasos que faltan (deploy de la migración/Edge Function, secret `RESEND_API_KEY`, corrida de verificación a mano).
- [x] **`/dashboard` pasado a `meeting_occurrences` — 2026-09-17.** `app/dashboard/page.tsx` ahora lee `getUpcomingOccurrences()` (`lib/automation.ts`, filtrado a `status==='synced'` con el mismo margen de 2h post-inicio que ya usaba `getUpcomingMeetings`) en vez de la tabla vieja `meetings`, con un componente nuevo de solo lectura (`components/occurrence-meeting-card.tsx`) que reusa `OccurrenceWhatsAppShare` — el mensaje de WhatsApp que ve toda la congregación ahora incluye la agenda real de WOL (antes solo lo veía el admin en `/dashboard/automatizacion`). Los botones de admin del flujo manual ("Nueva Reunión"/"Importar CSV"/"Pegar desde Zoom") se sacaron de esta página — la gestión de admin pasa 100% a `/dashboard/automatizacion`. `lib/meetings.ts`/`lib/zoom-parser.ts`/`components/{meeting-form,csv-import,zoom-import-dialog}.tsx` **no se borraron**, quedan como fallback documentado, sin ningún botón que los dispare desde la UI.
- [ ] Borrar de verdad `zoom-import-dialog.tsx`/`meeting-form.tsx`/`csv-import.tsx`/`lib/zoom-parser.ts`/`lib/meetings.ts` (fallback ya retirado de la UI arriba, pendiente un ciclo más antes de borrar el código)
