...
- [x] `zoom-apply-dispatch`/auto-disparo: `reconcile` dispara el mismo mecanismo de "Sincronizar ahora" al terminar una corrida real (no dry-run). **Implementado, deployado y verificado 2026-09-13** — ver PROGRESS.md para el detalle completo de la corrida real (12 ocurrencias creadas, agenda real de WOL, 0 bloqueos).
  - **Bug real encontrado en la verificación, sin arreglar todavía**: los 12 jobs de `zoom_outbox` quedaron `pending` — Playwright no encuentra el botón "Schedule a Meeting" en Zoom (timeout). Descartada la hipótesis de que fuera por las dos corridas simultáneas (concurrencia — `reconcile` dispara una vez por schedule, así que un cron real siempre va a lanzar 2 `zoom-apply-browser` casi a la vez): una tercera corrida sola, sin concurrencia, falló igual desde el primer job. Apunta a que **la sesión de Zoom capturada expiró** — riesgo ya documentado desde Fase 2-bis. Los jobs siguen reintentando solos (backoff exponencial, sin causar daño) pero no van a aplicar nada hasta recapturar la sesión (`npm run zoom:capture-session`, requiere login interactivo del usuario). **Pendiente, ver PROGRESS.md.**
