...
- [x] `drift-check` reporta divergencias, nunca corrige solo — **diseñado e implementado 2026-09-17; verificado contra la cuenta real 2026-09-19** (run `35446059120`: 0 divergencias, tras corregir 3 bugs en la primera corrida — ver `PROGRESS.md` 2026-09-19). Corre en GitHub Actions (no pg_cron: Playwright no vive en Deno), cron cada 10hs (encadenado tras reconcile) + botón "Chequear divergencias ahora". Compara TODA la cuenta de Zoom (no solo lo que la automatización creó) contra `meeting_occurrences` y guarda el resultado en `drift_check_runs`; si encuentra algo, además manda un email por Resend (reusa la cuenta de Galpón Digital/`ferreterias`). Pendiente menor: observar la primera divergencia real (`mismatch`/`orphan_in_zoom`) para confirmar el mail de Resend, que solo se manda si hay al menos una.
