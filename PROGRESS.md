...
## 2026-09-17 (cierre) — `drift-check` diseñado e implementado (Fase 5, último ítem del roadmap)

Único ítem de Fase 5 que quedaba "sin diseñar" — ver `ZOOM_AUTOMATION.md` sección "Diseño de `drift-check`" para el detalle técnico completo. Acá solo el resumen de la sesión y lo que queda pendiente.

- Decisiones acordadas con el usuario antes de codear (mismo criterio de este proyecto para toda feature grande): corre en GitHub Actions con cron semanal propio (no pg_cron — Playwright no vive en Deno); alcance amplio, TODA la cuenta de Zoom, no solo lo que la automatización creó (a propósito: el usuario quiere ver también las reuniones del flujo manual viejo que Zoom todavía tiene vivas); aviso por dashboard + email vía Resend solo si encuentra al menos una divergencia.
- **Credencial de email reusada de otro proyecto**: en vez de una cuenta de Resend propia para `my-app`, se reusa la que ya paga/mantiene `ferreterias` (Galpón Digital) — remitente confirmado leyendo `ferreterias/apps/api/.env` (`EMAIL_FROM=no-reply@galpon-digital.com.ar`, mostrado como "Galpón Digital <...>"), nunca pegado en el chat. El usuario va a copiar el valor real de `RESEND_API_KEY` él mismo como secret nuevo de este repo — no se le pidió generar uno nuevo, prefirió reusar el existente.
- Dos métodos nuevos de solo lectura en `ZoomBrowserClient` (`zoom-browser.ts`): `listMeetingIds()` (lista completa de la cuenta, reusa el selector de fila `a[href="/s/{id}"]` ya verificado como chequeo post-borrado en `cancelMeeting`) y `readMeetingSummary()` (abre "Editar" sin guardar nada, lee los mismos comboboxes que `create`/`updateMeeting` ya usan para escribir topic/fecha/hora/duración). Ninguno de los dos está verificado contra el DOM real todavía — mismo patrón que cualquier selector nuevo de este archivo, la primera corrida real se trata como verificación.
- Tabla nueva `drift_check_runs` (migración `20260917220000_drift_check_runs.sql`, mismo patrón de RLS que `reconcile_runs`/`zoom_session_checks`), script `zoom-automation/drift-check.ts`, workflow `.github/workflows/zoom-drift-check.yml`, Edge Function `zoom-drift-check-dispatch` (copia de `zoom-session-check-dispatch` apuntando a otro workflow), botón "Chequear divergencias ahora" + sección de divergencias en `/dashboard/automatizacion`.
